...
public final class MarkMyPlacesDBHelper extends SQLiteOpenHelper {
    ...
    protected int delete(String tableName, String whereClause, String[] whereArgs) {
        ...
        int updatedRows = 0;
        synchronized (writableDB) {
            updatedRows = writableDB.delete(tableName, whereClause, whereArgs);
        }
        return updatedRows;
    }
    ...
    public boolean deleteMyPlace(long rowId) {
        String whereClause = MyPlace.MyPlaceColumns._ID + " = ?";
        String[] whereArgs = {String.valueOf(rowId)};
        return delete(MyPlace.MyPlaceTable.TABLE_NAME, whereClause, whereArgs) > 0;
    }
    ...
    public Cursor queryGeoFencedPlacesAll(boolean isDateLatest) {
        String selection = MyPlace.MyPlaceColumns.IS_ADDED_FENCE + " = ? ";
    ...
    public Cursor queryMyPlace(long rowid) {
        String selection = MyPlace.MyPlaceColumns._ID + " = ? ";
