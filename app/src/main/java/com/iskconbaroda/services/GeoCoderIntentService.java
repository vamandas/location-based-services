...
public class GeoCoderIntentService extends IntentService {
    ...
    public static void launchGeocodeService(Context context, ResultReceiver resultReceiver, Location queryLocation) {
...
//                addTempDelay(10000);
    ...
    private QueryResult onQueryAddress(Intent queryIntent) {
        Location queryLocation = queryIntent.getParcelableExtra(EXTRA_LOCATION);
        ...
        if (queryLocation != null) {
            ...
            try {
                ...
                addresses = addressFetcher.getFromLocation(queryLocation.getLatitude(), queryLocation.getLongitude(), 1);
                if (isValidAddress(addresses)) {
                    result.resultCode = RESULT_SUCCESS;
                    result.queryAddress = getAddressString(addresses);
                } else {
                    result.errorMesage = getBaseContext().getString(R.string.no_location_data_provided);
                }
            } catch (IOException ioException) {
                // Catch network or other I/O problems.
                result.errorMesage = getString(R.string.no_geocoder_available);
                Log.e(TAG, result.errorMesage, ioException);
            } catch (IllegalArgumentException illegalArgumentException) {
                // Catch invalid latitude or longitude values.
                result.errorMesage = getString(R.string.invalid_lat_long_used);
                Log.e(TAG, result.errorMesage + ". " + "Latitude = " + queryLocation.getLatitude() + ", " + "Longitude = " + queryLocation.getLongitude(), illegalArgumentException);
            }
        } else {
            result.errorMesage = getBaseContext().getString(R.string.no_location_data_provided);
        }
